...
import com.cv.cvcorp.config.KafkaProperties;
...
class CvcorpKafkaResourceIT {
    ...
    void setup() {
        ...
        CvcorpKafkaResource kafkaResource = new CvcorpKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/cvcorp-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/cvcorp-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
